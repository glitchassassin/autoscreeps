...
type TerrainSource = Map<string, string> | Record<string, string> | ((roomName: string) => string | null | undefined);
...
export function installTestPathFinder(terrainSource: TerrainSource = {}): void {
...
export class TestCostMatrix {
  ...
  clone(): CostMatrix {
    return new TestCostMatrix(this.costs) as unknown as CostMatrix;
  }
  ...
  static deserialize(value: number[]): CostMatrix {
    return new TestCostMatrix(value) as unknown as CostMatrix;
  }
}

function createSearch(terrainSource: TerrainSource): PathFinder["search"] {
  return (origin, rawGoal, opts = {}) => {
    if (opts.flee) {
      throw new Error("Test PathFinder does not support flee searches.");
    }
...
function resolveTerrain(source: TerrainSource, roomName: string): string {
