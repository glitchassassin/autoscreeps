import { createTerrainDistanceTransform } from "../src/planning/distance-transform.ts";
...
async function main(): Promise<void> {
  ...
  process.stdout.write(`Distance-transform benchmark rooms: ${cases.length}\n`);
  ...
  process.stdout.write(`Metric: Chebyshev clearance from terrain walls and room bounds\n`);
...
function runSweep(cases: BenchmarkCase[], seed: number): number {
  ...
  for (const testCase of cases) {
    const transform = createTerrainDistanceTransform(testCase.terrain);
    checksum = foldChecksum(transform.distances, transform.maxDistance, checksum);
  }
...
function runSingleCase(testCase: BenchmarkCase, seed: number): number {
  const transform = createTerrainDistanceTransform(testCase.terrain);
  return foldChecksum(transform.distances, transform.maxDistance, seed);
}

function foldChecksum(distances: Uint8Array, maxDistance: number, seed: number): number {
  return (
    (seed * 33)
    ^ maxDistance
    ^ distances[0]!
...
function printHelp(): void {
  process.stdout.write(`Usage: npm run benchmark:distance-transform -- [room ...] [options]\n\n`);
