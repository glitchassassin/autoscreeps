...
import { planRoomStructures, type RoomStructurePlan } from "../src/planning/structure-plan.ts";
...
async function main(): Promise<void> {
  ...
  const { cases, failures } = loadBenchmarkCases(fixture.cases, config.roomNames);
  if (cases.length === 0) {
    throw new Error("No selected benchmark rooms produced a room structure plan.");
  }

  process.stdout.write(`Room-structure benchmark rooms: ${cases.length}/${cases.length + failures.length}\n`);
  process.stdout.write("Inputs: cached stamp fixtures + precomputed road, source/sink, and rampart plans\n");
...
function loadBenchmarkCases(cases: CachedStampPlanCase[], requestedRoomNames: string[]): {
  cases: BenchmarkCase[];
  ...
  for (const testCase of selectCases(cases, requestedRoomNames)) {
  ...
  return { cases: benchmarkCases, failures };
}
...
function measurePerRoom(cases: BenchmarkCase[], config: BenchmarkConfig, seed: number): {
  ...
  for (const testCase of cases) {
    const plan = planRoomStructures(testCase.room, testCase.stampPlan, testCase.roadPlan, testCase.sourceSinkPlan, testCase.rampartPlan);
    ...
    results.push({
      ...
      ramparts: counts.get("rampart") ?? 0,
      extensions: counts.get("extension") ?? 0,
      towers: counts.get("tower") ?? 0
    });
  }
...
function runSingleCase(testCase: BenchmarkCase, seed: number): number {
  return updateChecksum(seed, planRoomStructures(testCase.room, testCase.stampPlan, testCase.roadPlan, testCase.sourceSinkPlan, testCase.rampartPlan));
}

function updateChecksum(seed: number, plan: RoomStructurePlan): number {
  let checksum = (
    (seed * 33)
...
function printHelp(): void {
  process.stdout.write(`Usage: node scripts/benchmark-room-structures.ts [options] [room...]
